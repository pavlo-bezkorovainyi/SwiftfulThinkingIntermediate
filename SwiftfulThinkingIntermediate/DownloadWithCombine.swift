...
class DownloadWithCombineViewModel: ObservableObject {
  ...
  func getPosts() {
    ...
    //5. open and make sure the is correct
    //6. use the item
...
//      .subscribe(on: DispatchQueue.global(qos: .background)) //not necessary ???
      .receive(on: DispatchQueue.main)
      .tryMap(handleOutput)
      .decode(type: [PostModel].self, decoder: JSONDecoder())
      .replaceError(with: [])
//      .sink(receiveValue: <#T##(([PostModel]) -> Void)##(([PostModel]) -> Void)##([PostModel]) -> Void#>)
      .sink { (completion) in
//        switch completion {
//        case .finished:
//          print("finished")
